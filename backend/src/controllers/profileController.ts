...
export const getProfile = (req: AuthRequest, res: Response) => {
    ...
    db.query(query, [req.user?.id], (err, results: any[]) => {
...
export const updateProfile = (req: AuthRequest, res: Response) => {
    ...
    db.beginTransaction((err) => {
        ...
        db.query(query1, [name, bio, userId], (err) => {
            if (err) return db.rollback(() => res.status(500).json({ message: "Update failed" }));

            const query2 = "INSERT INTO audit_logs (user_id, action) VALUES (?, ?)";
            db.query(query2, [userId, "PROFILE_UPDATE"], (err) => {
                if (err) return db.rollback(() => res.status(500).json({ message: "Audit log failed" }));
